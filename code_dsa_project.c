...
void search_phone(FILE *fp);
void display(FILE *fp);
void phone_number_wise_sort(FILE *fp);
void name_wise_sort(FILE * fp);
void delete_contact();
void delete_all(FILE *fp,char filename[20]);
int count(FILE *fp);
void clean_stdin();
int checkAlpha(char str1[]);
...
int main()
{
    ...
    while(1)//defining menu
    {
        ...
        printf("\t\t\t4.Total Contacts in Phone Book\n\n");
        printf("\t\t\t5.Display Contacts Name Wise");
        printf("\t\t6.Display Contacts Phone Number Wise\n\n");
        printf("\t\t\t7.Search Contact By Name");
        printf("\t\t8.Search Contact By Phone Number\n\n");
        printf("\t\t\t9.Delete Existing Contact");
        printf("\t\t10.Delete all the Contacts\n\n");
        printf("\t\t\t11.Exit\n\n");
...
void add_contact(FILE *fp)//add new contacts in the directory
{
    ...
    scanf("%[^\n]s",&contact.name);
    while(checkAlpha(contact.name)!=1){
         fflush(stdin);
    ...
    while(1000000000>=contact.number || contact.number>=9999999999){
       printf("Contact number should be of 10 digits only\n");
       printf("Enter the contact number:");
       scanf("%lld",&contact.number);
    }
...
void search_name(FILE *fp)//search the contact of particular person by name
{
    int flag;
    char name_1[50];
    printf("Enter the name for contact details:");
    clean_stdin();
    scanf("%[^\n]s",name_1);
    rewind(fp);
...
void phone_number_wise_sort(FILE *fp)//sort contacts according to the phone numbers
{
    int i,j,k;
    k=0;
    rewind(fp);
...
void name_wise_sort(FILE *fp)//sort contacts according to the names in alphabetical order
{
